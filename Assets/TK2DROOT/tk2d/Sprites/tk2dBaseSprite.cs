...
public abstract class tk2dBaseSprite : MonoBehaviour, tk2dRuntime.ISpriteCollectionForceBuild
{
	...
	{ 
		...
		set 
		{
			if (value != _spriteId)
			{
				bool oldSpriteComplexGeometry = spriteId == -1 || collection.spriteDefinitions[spriteId].complexGeometry;
				...
				if (GetCurrentVertexCount() != collection.spriteDefinitions[value].positions.Length ||
					oldSpriteComplexGeometry != collection.spriteDefinitions[value].complexGeometry)
				{
					_spriteId = value;
					UpdateGeometry();
				}
